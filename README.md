...
- **AI 機能**: Vercel AI SDK + Supabase Queue (pgmq) によるバックグラウンド処理で、顧客メモリの自動生成と接客アドバイスを実装
- **型安全性**: DB 層（Drizzle ORM）から API 層（valibot + Result 型）まで、全レイヤーで型を一貫して保証
- **Server Components 活用**: Next.js 16 App Router の Parallel Routes や Server Actions を採用し、クライアントバンドルを最小化
- **テスト設計**: Google Testing Blog のテストサイズ分類に基づき、Small / Medium / Large / Browser テストを整備
...
| カテゴリ | 技術 | 選定理由 |
|---------|------|----------|
| フレームワーク | Next.js 16, React 19 | Parallel Routes、Server Actions を活用するため |
| 言語 | TypeScript 5.9 (Strict) | コンパイル時に型エラーを検出し、実行時エラーを減らす |
| スタイリング | Tailwind CSS 4, shadcn/ui | Radix UI ベースでアクセシビリティを確保 |
| DB | PostgreSQL, Drizzle ORM | `$inferSelect` で DB スキーマから型を自動生成 |
| バリデーション | valibot | Zod より軽量（~5KB）、`InferOutput` で型生成可能 |
| フォーム | react-hook-form | 非制御フォームで再レンダリングを抑制 |
| テスト | Vitest Browser Mode, Playwright | jsdom ではなく実ブラウザでテスト実行 |
| CI/CD | GitHub Actions, Vercel | PR ごとにプレビューデプロイとテストを自動実行 |
| AI | Vercel AI SDK | ストリーミングと構造化出力に対応 |
| キュー | Supabase Queue (pgmq) | AI 処理のバックグラウンド実行と Exponential Backoff リトライ |
| ロギング | pino | 構造化ログ、センシティブ情報の自動マスキング |
| パッケージ管理 | pnpm Catalog Mode | `catalog:` で依存バージョンを一元管理 |
