...
データ取得部分やロジック部分はロジックを持たない Presenter コンポーネントとして切り出しててテスタビリティを向上する
...
| ツール           | 用途             | 補足                                                                                                                                |
| ---------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| ESLint           | Lint             | flat config + 豊富なプラグイン群で Next.js / React Hooks / Tailwind v4 (better-tailwindcss) / Unicorn / import-x など多角的にカバー |
| Prettier         | Format           | コード整形のデファクト。plugin で shell / Dockerfile / .env / TOML / SQL / package.json まで対象を拡張しモノレポ全体を一括整形      |
