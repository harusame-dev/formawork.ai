...
- **型安全性**: 実行時エラーを減らすため、DB 層（Drizzle ORM）から API 層（valibot + Result 型）まで全レイヤーで型を一貫して保証
- **Server Components 活用**: クライアントバンドルを最小化するため、Next.js 16 App Router の Parallel Routes や Server Actions を採用
- **テスト設計**: 実装詳細ではなく振る舞いをテストするため、Google Testing Blog のテストサイズ分類に基づき Small / Medium / Large / Browser テストを整備
