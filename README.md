...
- 万が一アプリケーション層をバイパスされた場合のフェイルセーフとして機能
...
ランタイムではなく開発中に不具合を発見することで、不具合発見から修正までのリードタイムを短縮。「気をつける」ではなく仕組みで担保し、AI 駆動開発のガードレールとしても機能。
...
| tsc / next build | 型チェック | 代替なし。Go 版が安定したら移行予定 |

### CI/CD

ローカル環境固有の問題でエラーを見逃すリスクを排除するため、統一された環境でテストとコードチェックを実行。継続的な開発とチーム開発において必須の仕組み。

PR 時に実行: 静的解析、Browser テスト、Server テスト、E2E テスト、AI レビュー
マージ後: Vercel へ自動デプロイ
