...
| DB | PostgreSQL, Drizzle ORM | 軽量 ORM で SQL に近い API のため学習コストが低い。Bun や Cloudflare Workers など様々なランタイムで動作しポータビリティが高い |
| バリデーション | valibot | Zod と同等の機能で約 1/10 のバンドルサイズ（~5KB）。クライアントでも使用するためサイズは重要 |
| フォーム | react-hook-form | 非制御フォームで再レンダリングを最小化。大規模フォームでもパフォーマンス劣化しにくい |
...
| AI | Vercel AI SDK | 現在は Gemini のみ使用だが、GPT や Claude など別モデルへの切り替えを容易にするため採用。ストリーミング・構造化出力もサポート |
