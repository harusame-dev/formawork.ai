...
### テストでは実装の定数をインポートしない
テストで期待値として使用する定数は、実装からインポートせずにハードコードすること。

- ❌ 悪い例: `import { MAX_ITEMS } from "./constants"; expect(items).toHaveLength(MAX_ITEMS);`
- ✅ 良い例: `expect(items).toHaveLength(100);`
...
3. **実装とテストの独立性**: テストは実装から独立して仕様を検証する役割。実装に依存すると、その役割が果たせない
