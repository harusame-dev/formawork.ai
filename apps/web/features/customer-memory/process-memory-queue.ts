...
async function processCustomerMessages(
  ...
  if (result.success) {
    ...
    const archiveResults = await Promise.allSettled(
      messages.map((message) => memoryQueue.archiveMessage(message.msg_id)),
    );
  ...
  const deleteResults = await Promise.allSettled(
    messages.map((message) => memoryQueue.deleteMessage(message.msg_id)),
  );

  const deleteFailures = deleteResults
    .map((r, index) =>
      r.status === "rejected"
        ? { err: r.reason, msgId: messages[index]?.msg_id }
        : null,
    )
    .filter((f) => f !== null);
...
export async function processMemoryQueue(): Promise<void> {
  ...
  const groupedByCustomer = Object.groupBy(
    messages,
    (message) => message.message.customerId,
  );
