...
import { EditUserForm } from "./edit-user-form.client";

vi.mock("./register-user.action", () => ({
	registerUserAction: vi.fn(),
}));

vi.mock("@/features/user/edit/edit-user.action", () => ({
	editUserAction: vi.fn(),
}));

const test = base.extend<{
	registerUserActionMock: Mock;
}>({
	registerUserActionMock: async (
		// biome-ignore lint/correctness/noEmptyPattern: Vitestのfixtureパターンで使用する標準的な記法
		{},
		// biome-ignore lint/suspicious/noExplicitAny: https://github.com/vitest-dev/vitest/discussions/5710
		use: any,
	) => {
		const registerUserActionModule = await import("./register-user.action");
		await use(registerUserActionModule.registerUserAction);
		vi.clearAllMocks();
	},
});
...
test("送信中はボタンが無効化され、ローディング表示になる", async ({
	...
	registerUserActionMock.mockReturnValue(new Promise(() => {}));
