...
test("投稿日時が表示される", async () => {
	...
	const timeEl = page.getByRole("time");
...
test("未編集コメントに「編集済み」マークが表示されない", async () => {
	render(
		<TaskCommentsPresenter
			comments={[baseComment]}
			currentRole="user"
	...
	await expect.element(page.getByText("編集済み")).not.toBeInTheDocument();
});

test("編集済みコメントに「編集済み」マークが表示される", async () => {
	const editedComment = {
		...baseComment,
