...
test("姓名が空の場合、エラーが表示される", async () => {
	...
	// ハイドレーション完了を待つ
	await expect
		.element(page.getByLabelText("姓", { exact: true }))
		.not.toBeDisabled();
...
test("メールアドレスの形式が不正な場合、エラーが表示される", async () => {
	...
	// 不正な形式のメールアドレスを入力
	await page.getByLabelText("姓", { exact: true }).fill("テスト");
	await page.getByLabelText("名", { exact: true }).fill("太郎");
