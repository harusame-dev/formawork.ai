...
test("姓名が空の場合、エラーが表示される", async () => {
	...
	// ハイドレーション完了を待つ
	await expect.element(getLastNameInput()).not.toBeDisabled();
...
test("メールアドレスの形式が不正な場合、エラーが表示される", async () => {
	...
	// 不正な形式のメールアドレスを入力
	await getLastNameInput().fill("テスト");
	await getFirstNameInput().fill("太郎");
