...
const staffsConditionSearchParametersSchema = v.object({
...
export type StaffsConditionSearchParams = v.InferOutput<
  typeof staffsConditionSearchParametersSchema
>;

export type StaffsCondition = v.InferOutput<typeof staffsConditionSchema>;

export function parseStaffsConditionSearchParams(
  value: unknown,
): ReturnType<typeof succeed<StaffsConditionSearchParams>> {
  const result = v.safeParse(staffsConditionSearchParametersSchema, value);
...
export interface StaffsListItem {
  email: string;
  firstName: string;
  lastName: string;
  staffId: string;
