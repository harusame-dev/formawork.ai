...
interface EditCustomerNoteDialogProps {
  customerId: string;
...
export function EditCustomerNoteDialog({
  ...
  initialServiceDate,
  initialImages,
}: EditCustomerNoteDialogProps): React.JSX.Element {
  ...
  function handleRemoveExistingImage(path: string): void {
    setKeepImagePaths((previous) => previous.filter((p) => p !== path));
    setImageErrorMessage(null);
  }

  function handleRemoveNewImage(id: string): void {
    removeImage(id);
    setImageErrorMessage(null);
  }
