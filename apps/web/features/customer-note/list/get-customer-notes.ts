...
export async function getCustomerNotes(
	...
	// 画像パスを全ノートから収集し、signed URL とアドバイスを一括取得
	const allImagePaths = notesWithImages.flatMap((note) =>
		note.images.map((image) => image.path),
	);
	const allNoteIds = notesWithImages.map((note) => note.customerNoteId);

	const [imageUrlMap, adviceMap] = await Promise.all([
		getCustomerNoteImageUrls(allImagePaths),
		getLatestAdvices(allNoteIds),
	]);
