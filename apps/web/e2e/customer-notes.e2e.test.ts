...
test("正常系: 5枚の画像を含むノート登録成功", async ({ customerNotesPage }) => {
  ...
  await test.step("画像ギャラリーを開いて5枚の画像を確認", async () => {
    ...
    for (let index = 2; index <= 5; index++) {
      ...
      await expect(drawer.getByText(`${index} / 5`)).toBeVisible();
      // 前の画像が表示されていないことを確認
      await expect(
        drawer.getByRole("img", { name: `添付-${index - 1}` }),
      ).not.toBeInViewport();
      await expect(
        drawer.getByRole("img", { name: `添付-${index}` }),
      ).toBeInViewport();
    }

    // 前のボタンで戻れることを確認
    const previousButton = drawer.getByRole("button", {
      name: "前のスライド",
    });
