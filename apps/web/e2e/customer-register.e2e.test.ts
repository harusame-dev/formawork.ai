...
test("管理者が全フィールドを境界値一杯で入力して顧客を登録し、詳細ページへ遷移し、一覧画面で検索できる", async ({
  ...
  await test.step("フォームに境界値一杯のデータを入力", async () => {
    ...
    await page.getByLabel(/^姓(?!（かな）)/).fill(testData.lastName);
    // 負の先読みを使用して「名」だけにマッチさせる（「名（かな）」を除外）
    await page.getByLabel(/^名(?!（かな）)/).fill(testData.firstName);
  ...
  await test.step("登録した情報が正しく表示されることを確認", async () => {
    // getByRole('heading')でh1要素のみを対象にし、__next-route-announcer__との重複を回避
    await expect(
    ...
    const expectedPhone = testData.phone.replaceAll("-", "");
...
test("管理者が必須フィールドのみ入力して登録でき、詳細ページへ遷移する", async ({
  ...
  await test.step("登録した情報が正しく表示されることを確認", async () => {
    ...
    // 未登録：メール、電話番号、住所、生年月日、備考
    await expect(page.getByText("未登録")).toHaveCount(5);
  });
});
